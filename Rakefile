...
task :problem_8 do
  puts 'The four adjacent digits in the 1000-digit number that have the ' \
       'greatest product are 9 × 9 × 8 × 9 = 5832.'
  puts 'Find the thirteen adjacent digits in the 1000-digit number that have ' \
  ...
  ary = []
  (digits.length - 12).times do |t|
    ary << digits[(t)..(t+12)].scan(/\w/).map(&:to_i).inject(:*)
  end
  puts "The value is #{ary.max}"
end
